// SHA-256 in plain JavaScript, so the compiler produces the same digests (and
// therefore the same default seeds and certificates) in Node and in a browser.
...
const toBytes = (part) => (typeof part === "string" ? new TextEncoder().encode(part) : part);

// Digest of the concatenation of every part (strings are UTF-8), as lowercase hex.
export function sha256(...parts) {
  ...
  for (let block = 0; block < padded.length; block += 64) {
    ...
    for (let t = 16; t < 64; t++) {
      ...
      w[t] = (s2 + w[t - 7] + s1 + w[t - 16]) >>> 0;
    }
    let [A, B, C, D, E, F, G, H] = h;
    for (let t = 0; t < 64; t++) {
      const s = ((E >>> 6) | (E << 26)) ^ ((E >>> 11) | (E << 21)) ^ ((E >>> 25) | (E << 7));
      const t1 = (H + s + ((E & F) ^ (~E & G)) + K[t] + w[t]) >>> 0;
