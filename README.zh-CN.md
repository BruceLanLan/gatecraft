...
**它还可能告诉你：别用它。**这样实测过的六个真实决策里，有两个是模型在最有把握的地方也和人不一致——而这两个都通过了逐行证明、独立证明、重跑一致和高把握度。只有那二十道盲答抓住了它，所以这一步是闸，不是可选项。
...
依次点「Hold a comment」、「Read it back to me」、「fill it from the rule in this file」。这条路不用 key、不用注册任何账号，而且后面每一步都是真的：证明、能上手点的电路、二十道盲答、裁决和下载。
...
gatecraft 自己从不调用模型，也从不替你付钱；仓库里不带任何人的 key。要用 Jev，把 key 写在只有这台机器能读到的地方：
